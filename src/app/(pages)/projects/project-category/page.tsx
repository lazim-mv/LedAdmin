...
export default function Page() {
    const { projectCategories, refetch, isLoading, error } = useGetAllProjectCategories();
    ...
    const handleSaveSuccess = async (data: FormData, editorContent?: string, editorFormData?: EditorFormDataType, previewItemId?: string, clearCallback?: () => void) => {
        try {
            ...
            if (previewItemId) {
                response = await api.put(`project-categories/${previewItemId}`, transformedData);
            } else {
                response = await api.post("project-categories", transformedData);
            }
    ...
    return (
        <div className="appContainer">
            <main className="appMain">
                ...
                <BannerForm byIdEndPoint="project-categories" topBarTitle='Project Category'
                    title='Add New Project Category'
                    titlePlaceHolder='Category Name'
                    onSubmitSuccess={handleSaveSuccess}
                    allBanners={projectCategories}
